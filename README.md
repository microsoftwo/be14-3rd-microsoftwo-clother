
# 👕 Clother 👚

<img src="https://github.com/user-attachments/assets/8e8c7919-f4dd-45b0-9c94-faab9ea0231a">

<br><br><br>

---

# 🚀 Team microsoftwo 🚀

| <img src="https://github.com/user-attachments/assets/9e5c3e83-7f0d-41ca-816c-8133409537e1" width="200"> | <img src="https://github.com/user-attachments/assets/8f52dde1-3590-4e2f-880d-32098f5d098a" width="200"> | <img src="https://github.com/user-attachments/assets/0b56df7f-c48d-4975-abff-aeb51af8f2da" width="200"> | <img src="https://github.com/user-attachments/assets/f5cbf6cc-ff54-4720-be15-390182415083" width="200"> | <img src="https://github.com/user-attachments/assets/1d2619c6-f467-485e-9d90-14c7c43bc316" width="200"> |
| :-----------------------------------------------------------------------------------------------------: | :-----------------------------------------------------------------------------------------------------: | :-----------------------------------------------------------------------------------------------------: | :-----------------------------------------------------------------------------------------------------: | :-----------------------------------------------------------------------------------------------------: |
|                                                 김기종                                                  |                                                 조윤태                                                  |                                                 신민경                                                  |                                                 황수민                                                  |                                                 이예원                                                  |
| [<img src="https://img.shields.io/badge/Github-Link-181717?logo=Github">](https://github.com/rrangeess) |  [<img src="https://img.shields.io/badge/Github-Link-181717?logo=Github">](https://github.com/cxzaqq)   |  [<img src="https://img.shields.io/badge/Github-Link-181717?logo=Github">](https://github.com/mmmv41)   | [<img src="https://img.shields.io/badge/Github-Link-181717?logo=Github">](https://github.com/emily9949) |  [<img src="https://img.shields.io/badge/Github-Link-181717?logo=Github">](https://github.com/oni128)   |

<br><br>

---

# 📋 목차

<a href="#0">🎙️ 프로젝트 소개</a>

<a href="#1">☄️ 주요 기능</a>

<a href="#2">📚 기술 스택</a>

<a href="#3">📡 와이어프레임 </a>

<a href="#4">👀 스토리보드</a>

<a href="#5">📍 기능 명세서</a>

<a href="#6">📌 테스트 결과 보고서</a>

<a href="#7">🫡 동료평가</a>

<br><br>

---

# <p id="0">🎙️ 프로젝트 소개</p>

<img width="1000" alt="image" src="https://github.com/user-attachments/assets/9f076b1c-6b3d-48d5-a697-8a6837d88d05" />


<br><br>

---

# <p id="1">☄️ 주요 기능</p>


<img src = "https://github.com/user-attachments/assets/17495b98-d25a-4a48-af5b-3119b567dcf8">

<img src="https://github.com/user-attachments/assets/d7f8324d-d6df-493b-9dfa-0e1a10f3ad1d">

<img src="https://github.com/user-attachments/assets/28caa8f5-c9e0-44b3-b21c-097a86e4cc50">

- 코디뿐만 아니라 헤어, 메이크업, 악세서리까지의 스타일링을 공유할 수 있는 SNS

- 사용자는 자신이 원하는 스타일의 포스팅을 선택하고, 포스팅에 달린 태그를 통해 제품 정보 알 수 있고, 구매링크를 통해 바로 구매 혹은 예약처까지 연결될 수 있다.

- 포스팅 시에는 링크, 등록된 상품 등을 태그로 사용할 수 있고 원하는 상품이 없을 시 상품 등록 신청이 가능하다.

- StyleShare 게시판을 통해 자신의 스타일링에 대한 피드백을 받을 수 있다. 좋아요, 댓글 등 SNS의 기본적인 기능

<br><br>

---
# <p id="2">📚 기술 스택</p>

### FRONTEND
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Vue.js](https://img.shields.io/badge/Vue.js-4FC08D?style=for-the-badge&logo=vue.js&logoColor=white)
![Axios](https://img.shields.io/badge/Axios-5A29E4?style=for-the-badge&logo=axios&logoColor=white)
![GSAP](https://img.shields.io/badge/GSAP-88CE02?style=for-the-badge&logo=greensock&logoColor=white)


### DESIGN
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=for-the-badge&logo=figma&logoColor=white)


### DB
![MariaDB](https://img.shields.io/badge/MariaDB-003545?style=for-the-badge&logo=mariadb&logoColor=white)
![MyBatis](https://img.shields.io/badge/MyBatis-FFB725?style=for-the-badge&logo=MyBatis&logoColor=black)

### SERVER
![Eureka](https://img.shields.io/badge/Eureka-6528F7?style=for-the-badge)
![Gateway](https://img.shields.io/badge/Gateway-4D9BE6?style=for-the-badge)

### TOOL
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)
![IntelliJ IDEA](https://img.shields.io/badge/IntelliJIDEA-000000?style=for-the-badge&logo=intellijidea&logoColor=white)
![Notion](https://img.shields.io/badge/Notion-000000?style=for-the-badge&logo=notion&logoColor=white)



<br><br>

---

# <p id="3">📡 와이어프레임 </p>

<details>
    <summary>로그인 화면</summary>

####  로그인 화면 
<img src = "https://github.com/user-attachments/assets/5a6b0043-0e99-47bd-a65a-41b8eba2aba4">
</details>

<details>
    <summary> 포스팅 등록 화면 </summary>
    
####  포스팅 등록 화면
<img src = "https://github.com/user-attachments/assets/8db5e0ec-d20d-4ce6-b3c2-dd3488cb6ac5">
</details>

<details>

<summary> 마이페이지 화면 </summary>

#### 마이페이지 화면

<img src = "https://github.com/user-attachments/assets/60713110-f894-4282-8f37-43186ac94390">
    
</details>

<details>

<summary> 카테고리 별 포스팅 화면 </summary>

#### LOOK 카테고리 선택 시 화면

<img src = "https://github.com/user-attachments/assets/3a45410c-770a-46db-af1b-eb9176cb2dac">


#### MAKEUP 카테고리 선택 시 화면 

<img src = "https://github.com/user-attachments/assets/3fd7271b-cccb-4ee4-a536-2ff9903ad6f0">


#### HAIR 카테고리 선택 시 화면 

<img src = "https://github.com/user-attachments/assets/f0865880-25c9-43c7-a9cf-b2a80c84213a">
  
</details>

<details>

<summary> 스타일 쉐어 게시판 화면 </summary>

#### 스타일 쉐어 게시판 화면

<img src = "https://github.com/user-attachments/assets/8272d581-af3c-4d87-9c04-a489533f15af">

</details>

<details> 
    
<summary> 포스팅 상세 화면 </summary>

#### 포스팅 상세 화면

<img src = "https://github.com/user-attachments/assets/18ea81ca-669f-40a4-8f44-a08e5ce40053">

</details>


<br><br>

---

# <p id="4">👀 스토리보드</p>

<details>
    <summary>랜딩 화면</summary>
  
  #### 랜딩 화면
<img width="1150" alt="스크린샷 2025-04-19 22 19 40" src="https://github.com/user-attachments/assets/d6b9defb-9893-40b9-b1a3-ded0585fe556" />

</details>

<details>
  <summary>회원가입</summary>

  #### 회원가입 화면
  <img width="1150" alt="스크린샷 2025-04-16 07 45 10" src="https://github.com/user-attachments/assets/2557597b-f386-492f-aca0-44c384a8f13b" >


  #### 회원가입 성공 화면
  <img width="1150" alt="스크린샷 2025-04-19 21 56 40" src="https://github.com/user-attachments/assets/7e9e7bf7-2cae-4c88-890d-6adfb7458667" />
</details>

<details>
  <summary>로그인</summary>
  
  #### 로그인 화면
<img width="1150" alt="스크린샷 2025-04-19 22 21 12" src="https://github.com/user-attachments/assets/2ed2f059-6230-456e-b6c6-54e01dd98718" />

  #### 로그인 실패 화면
  <img width="1151" alt="스크린샷 2025-04-19 21 57 56" src="https://github.com/user-attachments/assets/e7d6e588-e83a-4481-a168-557797b07a66" />

</details>

<details>
  <summary>포스팅</summary>
  
  #### 메인페이지 화면
<img width="1150" alt="스크린샷 2025-04-19 22 07 57" src="https://github.com/user-attachments/assets/3e0738ee-8d7a-4840-b9d6-18084f1090a3" />

  #### 포스팅 조회 화면
<img width="1150" alt="스크린샷 2025-04-19 22 10 05" src="https://github.com/user-attachments/assets/59c0889a-9d4c-45db-a40d-28ddbfaf388e" />


  #### 포스팅 등록 화면
<img width="1149" alt="스크린샷 2025-04-19 22 10 43" src="https://github.com/user-attachments/assets/0c53812c-3a11-4919-8402-d24473c15f00" />

</details>

<details>
  <summary>스타일 쉐어</summary>
  
  #### 스타일 쉐어 게시판 화면
<img width="1146" alt="스크린샷 2025-04-19 22 04 16" src="https://github.com/user-attachments/assets/454eae00-12f5-4698-93f7-b0eec96d2a20" />

  #### 스타일 쉐어 게시물 화면
<img width="1145" alt="스크린샷 2025-04-19 22 05 47" src="https://github.com/user-attachments/assets/9a8eca29-4ee4-44f3-88bb-02873c9c0975" />


  #### 스타일 쉐어 게시물 댓글 화면
<img width="1147" alt="스크린샷 2025-04-19 22 07 02" src="https://github.com/user-attachments/assets/cde7f7dd-d594-4fb8-be27-f8832acee1e4" />


  #### 스타일 쉐어 게시물 등록 화면
<img width="1146" alt="스크린샷 2025-04-19 22 22 40" src="https://github.com/user-attachments/assets/ffec02ca-3129-4f84-9751-1ee5e3711bef" />


</details>


<details>
  <summary>마이페이지</summary>
  
  #### 마이페이지 화면
<img width="1152" alt="스크린샷 2025-04-19 22 00 18" src="https://github.com/user-attachments/assets/ec8e04af-46bf-4eba-be47-ac00e8372cf9" />

  #### 회원정보 수정 화면
<img width="1149" alt="스크린샷 2025-04-19 22 01 10" src="https://github.com/user-attachments/assets/dd910f46-2ccf-4ec7-870e-0dc97d62dadb" />

  #### 회원정보 수정 성공 화면
<img width="1150" alt="스크린샷 2025-04-19 22 01 27" src="https://github.com/user-attachments/assets/80fe1850-2a3e-4d5b-a7ac-a21af59681f2" />

  #### 나의 활동 기록
<img width="1147" alt="스크린샷 2025-04-19 22 02 39" src="https://github.com/user-attachments/assets/c8ffa48a-48ed-41ae-bd53-44e3dc6e2930" />

  #### 로그아웃
<img width="1150" alt="스크린샷 2025-04-19 22 24 01" src="https://github.com/user-attachments/assets/a1440732-fe53-4db6-b6ac-5f857129f3e4" />

</details>

<details>
  <summary>공통 레이아웃</summary>
  
  #### 헤더
<img width="1150" alt="스크린샷 2025-04-19 22 16 09" src="https://github.com/user-attachments/assets/5816c181-b548-4e0c-ae50-58fd4cb6e31a" />

  #### 푸터
<img width="1150" alt="스크린샷 2025-04-19 22 16 30" src="https://github.com/user-attachments/assets/9e496558-c5e5-4546-a992-91be6914d73c" />

</details>


<br><br>

---

# <p id="5">📍 기능 명세서</p>


[[🔗기능 명세서 Link]](https://docs.google.com/spreadsheets/d/1HQYulWkDctMpm4mhM3wYKbtAY3fCOOigEeM-M-q0jlg/edit?usp=sharing)
<br>

<img width="1344" alt="image" src="https://github.com/user-attachments/assets/69ad7243-289e-4f98-9a99-43ccfc92ac10" />
<img width="1232" alt="image" src="https://github.com/user-attachments/assets/2d44f2f3-8f2f-4e11-b651-59e261c63a4e" />
<img width="1232" alt="image" src="https://github.com/user-attachments/assets/c64909dd-0cb8-4477-b3fc-b6df51671990" />

<br><br>

---

# <p id="6">📌 테스트 결과 보고서</p>

테스트 결과에 대한 자세한 내용과 시연 영상은 아래 노션 링크에서 확인하실 수 있습니다.  

[[🔗테스트 결과 보고서 Link]](https://ohgiraffers.notion.site/1d6649136c11805c884cd7559d1ab25b?pvs=4)

<br><br>

---

# <p id="7">🫡 동료평가 </p>

<details>
<summary>동료 평가 확인하기</summary>
<div markdown="1">

#### ABOUT [김기종](https://github.com/rrangeess)

|  FROM  | COMMENT                                                                                                                                                                                                                                                                                                              |
| :----: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 조윤태 | 특정 기능이나 기술적인 부분에서 흥미를 느끼면 깊이 있게 파고드는 모습이 인상적이었습니다. 단순히 관심을 갖는 데 그치지 않고, 끊임없이 고민하며 결국 해결해내는 집념에서 개발자로서의 성장 가능성이 크게 느껴졌습니다. 또한 UI 발표를 훌륭하게 준비해주셔서 팀의 기획 의도와 구현 방향이 잘 전달될 수 있었던 점도 감사하게 생각합니다. 다음에 기회가 된다면 또 같이 프로젝트를 진행하고 싶은 팀원입니다. |
| 신민경 | 맡은 역할에 항상 최선을 다하며 책임감을 가지고 임하는 모습이 보기 좋았습니다. 본인이 구현한 부분에 대해 지속적으로 고민하고, 더 나은 방향으로 개선해 나가려는 모습 덕분에 더 좋은 결과물이 나올 수 있었다고 생각합니다. 주어진 역할 이상으로 프로젝트의 완성도를 높이기 위한 노력이 돋보였던 팀원이었습니다. |
| 황수민 |  먼 거리에도 불구하고 대면 회의에 적극적으로 참여하려는 노력에 감탄했습니다. 체력적으로 힘들었을 텐데도 밝은 태도로 분위기를 이끌어주신 덕분에 팀원들 모두 끝까지 집중할 수 있었습니다. 특히, 발표 직전까지도 더 나은 발표를 위해 고민하고 구성해주신 덕분에, 더욱 짜임새 있는 발표 자료를 완성할 수 있었습니다. 또한, 팀원들 간의 이해도가 다를 때 기종님께서 개념을 명확하게 정리해주신 덕분에 모호한 부분을 확실히 짚고 넘어갈 수 있었습니다. 개발 과정에서도 맡은 역할을 철저히 수행해주셔서 프로젝트에 큰 도움이 되었습니다!|
| 이예원 | 본인이 맡은 기능을 책임감 있게 완성도 높게 구현해주었습니다. 사진 미리보기, 삭제 시 이미지 재정렬 등 난이도 있는 기능을 안정적으로 구현하고, S3를 활용한 이미지 저장 기능까지 성공적으로 구성하며 서비스 흐름을 매끄럽게 이끌어주었습니다. 또한 UI 발표를 맡아 기획 의도와 사용자 흐름을 논리적이고 명확하게 전달해, 프로젝트에 대한 이해도를 높이고 발표의 설득력을 강화하는 데 큰 역할을 했습니다. 기술 구현과 커뮤니케이션 양면에서 팀의 완성도를 높여준 팀원입니다. |

#### ABOUT [조윤태](https://github.com/cxzaqq)

|  FROM  | COMMENT                                                                                                                                                                                                                                                                                                          |
| :----: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 김기종 | 프로젝트의 흥망성쇠가 윤태 님에게 달렸을 정도로 큰 역할과 부담을 가지셨는데, 그럴 때마다 긍정적이고 성실한 태도로 훌륭한 결과물이 도출되게 임해주셨습니다. 이전 프로젝트에서의 백엔드 개발을 토대로 프론트엔드와의 연동, 연동을 위한 gateway 설정, CORS 에러 방지를 위한 설정 등 막중한 업무들을 구현해 주심뿐만 아니라 문제가 생긴 팀원들의 질문도 자신의 문제처럼 귀 기울여 주시고 같이 고민, 해결해 주시는 모습도 보여주셨습니다. 이처럼 기술 구현과 팀원과의 조화 모두 완벽하게 챙겨주셔서 덕분에 저 또한 성장하게 될 수 있는 계기가 되었습니다. 감사합니다!    |
| 신민경 | 프론트엔드 구현부터 백엔드 연동까지 폭넓은 영역을 아우르며 프로젝트에 큰 기여를 해주신 팀원입니다. 특히 드래그 앤 드롭을 이용한 상품 태그 기능을 완성도 높게 구현해 주셔서 서비스의 전반적인 완성도가 크게 향상되었으며, 기획했던 결과물 그대로 나올 수 있었습니다. 또한, 프론트엔드 경험이 부족한 팀원들을 위해 기꺼이 본인의 지식을 공유해주셔서 프로젝트가 원활히 진행될 수 있었던 것 같습니다. |                                                                 
| 황수민 |  팀의 실질적인 리더 역할을 해주셨다고 생각합니다. 프로젝트 도중 난관에 봉착했을 때도 침착하게 해결 방안을 모색하며 더 나은 방향으로 이끌어주셨습니다. 기술적인 이해도가 뛰어나 많은 도움을 받을 수 있었고, 팀원들의 질문에도 함께 고민하며 적극적으로 문제 해결을 하려고 노력해주셨습니다. 문제가 생겼을 때 바로바로 해결방안을 도출해주신 윤태님 덕분에 프로젝트 기간 동안 팀원들도 함꼐 기술적 성장을 이루어낼 수 있었던 것 같습니다. 많은 일을 맡으셨음에도 불구하고 힘든 내색 없이 끝까지 책임감 있게 임하셨으며, 겸손한 태도를 유지하는 모습이 인상적이었습니다. 특히, 강한 책임감으로 발표까지 완성도 높게 마무리하신 점은 정말 박수쳐드리고 싶습니다!|
| 이예원 | 난이도 높은 기능들을 안정적으로 구현하며 프로젝트 전반의 완성도 향상에 큰 영향을 미쳤습니다. 사진 미리보기, 삭제 시 이미지 재정렬 처리, 이미지 위 상품 태그의 드래그앤드랍 좌표 지정 등 복잡한 기능들을 책임감 있게 구현했으며, S3를 활용한 이미지 저장 및 활용까지 직접 구현하여 핵심 기능의 흐름을 안정적으로 구축했습니다.맡은 영역에 국한되지 않고, 필요한 작업을 찾아 해결하며 프로젝트 전반의 품질을 높이는 데 기여해주었고, 팀원들의 상황을 세심하게 살피며 팀원들의 어려움에 귀 기울이고 먼저 다가가주는 따뜻한 태도 덕분에 팀 분위기가 훨씬 부드럽고 안정적이었습니다. 프로젝트 전반을 넓은 시야로 바라보며 케어해주는 모습이 정말 인상 깊었습니다. |

#### ABOUT [신민경](https://github.com/mmmv41)

|  FROM  | COMMENT                                                                                                                                                                                                                                                                                                          |
| :----: | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 김기종 | 이번 프론트엔드 프로젝트의 일등공신이라고 생각합니다. 프로젝트의 취지와 목적에 완벽하게 부합하는 디자인을 만들기 위해 밤낮없이 노력하였고, 이를 통해서 눈을 사로잡는 디자인이 나왔다고 생각합니다. 잘 짜인 디자인 덕분에 프론트엔드의 구현 과정이 원활하고 빠르게 진행됐습니다. 또한 랜딩 화면과 홈 화면과 같은 시각적인 효과가 중요한 부분에서도 library와 tool을 활용해 멋진 결과물을 도출해 줬습니다. 이를 구현하는 과정에서 상당한 양의 더미 데이터를 쌓는 등의 귀찮은 일들을 피하지 않고 책임감 있게 맡는 모습이 인상 깊었고 감사합니다!   |
| 조윤태 | 맡은 역할에 대한 책임감이 매우 강한 팀원이었습니다. 어려운 부분이 있어도 쉽게 포기하지 않고 끝까지 해결하려는 끈기 있는 모습이 인상 깊었으며, 실제로 주어진 작업을 성실하게 마무리하여 팀 전체 일정에도 큰 도움이 되었습니다. 꾸준하고 믿음직한 태도 덕분에 팀원들도 안심하고 함께 협업할 수 있었고, 문제 해결 과정에서 보여준 집중력과 집요함은 프로젝트의 완성도를 높이는 데에도 크게 기여했다고 생각합니다. |
| 황수민 | 기술적인 구현뿐만 아니라 기획 단계에서도 적극적으로 아이디어를 제시해 주셨고, 문서화 작업까지 꼼꼼하게 챙겨주셔서 정말 만능 개발자라고 느꼈습니다! 또한, 팀 분위기를 한층 밝게 만들어주셔서 덕분에 화기애애한 분위기 속에서 프로젝트를 진행할 수 있었습니다. 민경님 덕분에 팀원들의 단합력도 더욱 높아졌습니다! 프로젝트 기간 동안 많은 의지가 되었습니다 |
| 이예원 | 사용자가 서비스에 자연스럽게 흥미를 느낄 수 있도록 유도하는 랜딩 페이지와 메인 페이지를 완성도 높게 구현하였습니다.특히 랜딩 페이지에서는 GSAP과 ScrollTrigger를 활용해 부드럽고 역동적인 스크롤 인터랙션을 구현함으로써, 시각적 몰입감과 사용자 경험을 크게 향상시켰습니다. 또한 피그마를 활용한 디자인 구상 단계부터 실제 구현에 이르기까지, 디자인에 대한 뛰어난 감각을 바탕으로 전체적인 화면 구성, 색감, 애니메이션 흐름 등을 표현해주었습니다. 단순한 화면 구현을 넘어, 서비스의 방향성과 정체성을 시각적으로 잘 표현해낸 점에서 큰 기여를 했다고 생각합니다. |

#### ABOUT [황수민](https://github.com/emily9949)

|  FROM  | COMMENT                                                                                                                                                                                                                                                                                                                                                              |
| :----: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 김기종 | 주어진 작업을 막힘없이 수월하게 끝내는 에이스라고 생각했습니다. 남들보다 빠른 시간 안에 구현을 척척해내는 모습을 보면, 이해력이 빠르고 실력이 출중하다고 느꼈습니다. 디자인 설계 과정에서도 밤낮없이 노력해 주고, 프론트엔드 구현 시에도 먼저 템플릿을 만들어주어 시작이 매끄러워질 수 있었다고 생각했습니다. 또한 팀원들과 의견을 공유하는 과정에서도 상대의 의견을 존중해 주고 경청해 주는 태도를 보여주신 덕분에 프로젝트가 원활하게 흘러갈 수 있었다고 생각합니다. 기술 구현과 팀원 조화 두 가지 모두 완벽히 수행해 주신 점에 감사합니다 ! |
| 조윤태 | 뛰어난 디자인 감각을 바탕으로 전반적인 UI의 기반을 설계해 주셔서 팀원들에게 큰 도움이 되었습니다. 세심한 화면 구성과 시각화 덕분에 프로젝트 결과물뿐만 아니라 발표 자료 또한 높은 퀄리티로 완성할 수 있었고, 팀 전체가 자신 있게 내세울 수 있는 프로젝트가 되었던 것 같습니다. 또한, 백엔드에 대한 이해도도 높아 이러한 역량을 바탕으로 앞으로 훌륭한 풀스택 개발자로서의 성장이 기대되는 팀원입니다. |
| 신민경 | 디자인 구현 과정에서 사소한 부분도 놓치지 않고 꼼꼼히 진행하는 모습 덕분에 전체적인 퀄리티가 한층 더 높아졌습니다. 본인이 맡은 작업을 빠르게 처리한 뒤, 다른 팀원들을 적극적으로 도와주거나 누구도 맡지 않은 파트를 묵묵히 수행해주었습니다. 또한, 특유의 디자인 감각과 센스, 그리고 많은 아이디어 제공으로 서비스의 비주얼적인 완성도가 크게 향상시켜준 팀원입니다.  |                                                                              
| 이예원 | 피그마를 활용한 디자인 설계 과정에서 뛰어난 감각을 보여주었으며, 이를 실제 구현 단계에서도 거의 완벽하게 반영해내는 높은 구현력을 갖추고 있습니다. 특히 헤더와 푸터 등의 템플릿을 먼저 구성해 전체적인 구조를 잡아줌으로써, 팀원들이 각자 작업을 매끄럽게 시작할 수 있도록 도와준 점이 인상 깊었습니다. 또한 발표 자료 역시 시각적 완성도와 전달력이 뛰어나, 프로젝트를 명확하고 효과적으로 소개하는 데 큰 역할을 했습니다. 기획, 구현, 발표 모든 면에서 꼼꼼함과 감각을 두루 갖춘 팀원입니다. |

#### ABOUT [이예원](https://github.com/oni128)

|  FROM  | COMMENT                                                                                                                                                                                                                                                                                                                                       |
| :----: | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 김기종 | 누구보다 열심히 하고 어려운 문제를 끝까지 해내는 모습이 멋있었습니다. 이번 프로젝트에서 서버와의 연동, 라우터 설정 등 복잡한 상황에 부딪힐 때마다 피하지 않고 어떻게든 해결해 내려는 의지를 갖고 맞서는 모습이 인상 깊었습니다. 또한 결국 해내는 모습에서 큰 동기부여를 받았습니다. 프로젝트가 장기화될수록 몸 상태가 악화되는 게 보여 걱정이 많았습니다. 그럼에도 불구하고 항상 밝은 모습으로 맡은 바에 끝까지 집중하는 태도가 예원 님의 가장 큰 장점이자, 본받고 싶은 점이라는 생각이 듭니다. 이번 프로젝트 동안 고생하셨고, 얼른 나으시길 바랍니다. 감사합니다!  |
| 조윤태 | 프론트엔드 프로젝트 발표를 맡아주셨는데 정말 훌륭하게 마무리해주셨습니다. 본인이 직접 개발하지 않은 영역도 적극적으로 학습하여 내용을 이해하고 발표에 임했던 자세가 인상적이었습니다. 본인이 맡은 부분도 훌륭하게 구현해 내셨기에 프로젝트 전체에 대한 이해도와 기여도가 높았습니다. 또한 늘 긍정적인 모습을 잃지 않아 기분 좋게 프로젝트를 진행했습니다. 발표자로서도, 개발자로서도 모두 믿음직한 팀원입니다. |
| 신민경 | 항상 다양한 부분에 호기심을 가지고 적극적으로 임하는 자세가 인상적인 팀원이었습니다. 맡은 파트에 대해 깊이 이해하고자 노력하며, 더 나은 결과물을 만들기 위해 끊임없이 고민하는 모습이 인상 깊었습니다. 또한, 프로젝트 진행 중 겪은 어려운 상황에서도 쉽게 포기하지 않고 밤을 새서라도 끝까지 해내는 끈기 있는 태도가 정말 멋졌습니다. |
| 황수민 | 예원님은 프로젝트 기간 동안 놀라운 의지력과 노력을 보여주셨습니다. 맡은 기능을 다 구현할 수 있을지 걱정하셨지만, 그런 걱정이 무색할 정도로 가장 먼저 기능을 완성해내는 모습을 보며 강한 의지와 집요한 문제 해결력이 돋보인다고 느꼈습니다! 힘든 상황에서도 웃음을 잃지 않고 팀원들에게 긍정적인 에너지를 전해주신 덕분에 즐겁게 프로젝트에 임할 수 있었습니다. 묵묵히 해야 할 일을 찾아 수행해주신 덕분에 팀원들의 부담도 한층 덜어질 수 있었어요. 예원님 덕분에 긍정적인 힘을 많이 얻었습니다 감사합니다! |

</details>

